...
/**
 * DeductCreditsCommand — атомарне списання кредитів з балансу користувача.
 *
 * Витягнуто з CreditService::deduct(). Поведінка, повідомлення про помилки
...
class DeductCreditsCommand
{
     ...
     *
     * @return bool Успішність (true навіть при дублікаті — баланс не змінюється)
    ...
    public function execute(
        ...
        return TransactionManager::run(function (\PDO $pdo) use (
            ...
            error_log(
                "CreditService deduct accepted user={$user->id} amount={$amount} type={$type}"
